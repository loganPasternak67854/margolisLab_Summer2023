function[]=overlayDisplay_withThresholding(images,struct,data,num,refNum,revamped_sessions)
%{
...
%If reference image is not from Session 1 execute the code below
    ...
    %Select that Daas and ROI that you want to set as a control
    statement1="Select a Daas other than Daas";
    statement2=num2str(refNum);
    statement3="...\n";
    combinedStatement=append(statement1,statement2,statement3);

    in=input(combinedStatement);
    choice=cast(in,"uint8");
    fprintf("Choose an ROI in the selected Daas\n");
    result=roiSelection(data,choice);
    ...
    %Prep for overlay
    img1=cast(images(refNum,num).window,"uint8");
    mask= img1~=0;
    img1(mask)=255;
    img2=cast(img2,"uint8");
    rgbImage=labeloverlay(img1,img2);
